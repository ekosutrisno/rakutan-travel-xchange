...
<div class="flex-1">
    ...
    <div v-if="results.length" class="px-4 pb-4">
        <h4 class="font-bold text-[14px]">Singapore: {{results.length}} properties found</h4>
    </div>


    <!-- Placeholder session when loading fetch api -->
    <div v-if="loading" class="pt-2.5">
        <PropertyCardPlaceholderSmall v-for="idx in 3" :key="idx" />
    </div>
    ...
    <div class="grid gap-2 p-[10px]">
        ...
        />
        <!-- If property empty -->
        <div v-if="!results.length && !loading && !error" class="text-center flex flex-col mt-[18px]">
            <div class="mx-auto">
            ...
            <h3 class="font-bold text-[14px] mt-[12px] text-placeholder">Sorry! We couldn't find any properties for your search.</h3>
        </div>

        <!-- If Bad request status code 4xx -->
        <div v-if="error" class="text-center flex flex-col mt-[18px]">
            <div class="mx-auto">
            ...
            <h3 class="font-bold mt-[12px] text-placeholder">Opps, something went wrong.</h3>
